"use strict";
function runSpiceTest() {
    const select = document.getElementById('spiceQuiz');
    const resultBox = document.getElementById('spiceResult');
    if (!select || !resultBox)
        return;
...
function calculateTax() {
    const input = document.getElementById('momoCount');
...
function toggleAnthem() {
    ...
    const status = document.getElementById('anthemStatus');
    if (!btn || !eq || !status)
        return;
    isPlaying = !isPlaying;
    if (isPlaying) {
...
window.runSpiceTest = runSpiceTest;
window.calculateTax = calculateTax;
window.toggleAnthem = toggleAnthem;
